...
import {BehaviorSubject, catchError, combineLatest, forkJoin, map, mergeMap, Observable, of, switchMap} from 'rxjs';
...
@Component({
  ...
  imports: [LucideAngularModule, NgIf, AsyncPipe, NgForOf, RouterLink],
...
export class WatchedComponent implements OnInit{
    ...
    this.filteredMovies$ = combineLatest([
      ...
      map(([movies, selectedDecade, selectedGenre, selectedSort]) => {
        ...
        if (selectedDecade) {
          ...
          const endDate = new Date(startYear + 10, 0, 1);
  ...
  getWatchedMoviesPosters(): Observable<any> {
    return this.movieService.getWatchedOrWatchlist(this.authService.getCurrentUser()?._id, 'WATCHED').pipe(
